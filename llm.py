...
class ChatBot():
  ...
  def generate_subquestions(self, question: str):
    ...
    oneshot_example = HumanMessage(content="""
      Generate 3 to 4 sub-queries based on this initial job description. Return the answer as a python list:
    ...
    res = self.llm.invoke([system_message, oneshot_example, oneshot_response, user_message]).content.strip()
    return eval(res)
  ...
  def generate_message_stream(self, question: str, docs: list, history: list, type: str):
    ...
    if type == "1":
      system_message = SystemMessage(content="""
        ...
        Use the resumes given in the context and summarize them in your answer and/or answer the specific question given by the user. 
